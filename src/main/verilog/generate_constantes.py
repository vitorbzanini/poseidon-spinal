...
def output_sv_constants(t):
    """Gera as constantes e escreve no formato de array do SystemVerilog"""
    
    r_f = basic.ROUNDFULL
    r_p = basic.ROUNDPARTIAL[t]
    round_constants = generate_constants(t, r_f, r_p)
    total_constants = len(round_constants)
    
    filename = f"poseidon_constants_t{t}.sv"
    with open(filename, "w") as f:
        f.write(f"// Constantes geradas para Poseidon t={t}, RF={r_f}, RP={r_p}\n")
        f.write(f"// Total de constantes: {total_constants}\n")
        f.write(f"localparam [254:0] ROUND_CONSTANTS [0:{total_constants-1}] = '{{\n")
        
        for i, element in enumerate(round_constants):
            comma = "," if i < total_constants - 1 else ""
            f.write(f"    255'h{element:064x}{comma}\n")
            
        f.write("};\n")
        
    print(f"Sucesso! {filename} gerado pronto para o RTL.")




if __name__ == "__main__":
    # Chama a função especificando t=9
    output_sv_constants(9)